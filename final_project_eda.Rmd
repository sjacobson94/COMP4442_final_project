...
covid_2020 <- covid_texts %>%
  ...
  ungroup() %>%
  filter(stri_enc_isascii(abstract), stri_enc_isascii(title)) %>%
  rowwise() %>%
  mutate(
...
```{r}
abstract_tokens_raw <- covid_2020[c('title','abstract')] %>%
...
abstract_tokens <- abstract_tokens_raw %>%
